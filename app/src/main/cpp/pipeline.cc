...
bool Pipeline::ProcessImg(const std::string &imagePath) {
    __android_log_print(ANDROID_LOG_DEBUG, "ABC", "Loading image from: %s", imagePath.c_str());

    cv::Mat bgrImage = cv::imread(imagePath);
    if (bgrImage.empty()) {
        __android_log_print(ANDROID_LOG_DEBUG, "ABC", "Can not load image");
        return false;
    }

    int height = 448;
    int width = 448;
    cv::Mat bgrImageResize;
    cv::resize(bgrImage, bgrImageResize, cv::Size(width, height));
    LOGD("Image resized to: %d x %d", width, height);

    auto boxes = detPredictor_->Predict(bgrImageResize, Config_, nullptr, nullptr, nullptr);
    LOGD("Image resized to: %d x %d", width, height);

    std::vector<std::string> recText;
    std::vector<float> recTextScore;
    for (int i = boxes.size() - 1; i >= 0; i--) {
        LOGD("Image resized to: %d x %d", width, height);

        cv::Mat cropImg = GetRotateCropImage(bgrImageResize, boxes[i]);
        if (cropImg.empty()) {
            LOGD("Image resized to: %d x %d", width, height);
            continue;
        }
        auto res = recPredictor_->Predict(cropImg, nullptr, nullptr, nullptr, charactor_dict_);
        recText.push_back(res.first);
        recTextScore.push_back(res.second);
        LOGD("Recognized text: %s, confidence: %f", res.first.c_str(), res.second);

    }

    Visualization(bgrImageResize, boxes, imagePath);
    __android_log_print(ANDROID_LOG_DEBUG, "ABC", "Image loaded successfully from: %s", imagePath.c_str());
