...
JNIEXPORT jboolean JNICALL
Java_com_baidu_paddle_lite_demo_ppocr_1demo_Native_nativeProcessImg(
        JNIEnv *env, jclass thiz, jlong ctx, jstring jsavedImagePath) {
    if (ctx == 0) {
        return JNI_FALSE;
    }
    std::string savedImagePath = jstring_to_cpp_string(env, jsavedImagePath);
    Pipeline *pipeline = reinterpret_cast<Pipeline *>(ctx);
    return pipeline->ProcessImg(savedImagePath);
}
/*
 * Class:     com_baidu_paddle_lite_demo_ocr_db_crnn_Native
